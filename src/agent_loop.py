    ...
    for round_num in range(1, max_rounds + 1):
        ...
        if not tool_blocks:
            ...
            if _promise_shape and _intent_nudge_count >= _MAX_INTENT_NUDGES:
                ...
                yield "data: " + json.dumps({"type": "intent_nudge_exhausted", "round": round_num, "nudges": _intent_nudge_count, "max_nudges": _MAX_INTENT_NUDGES, "message": _in_message}) + "\n\n"
            break  # no tools — done
        ...
        if _stuck_rounds >= _MAX_STUCK_ROUNDS or _runaway:
            ...
            yield "data: " + json.dumps({"type": "loop_breaker_triggered", "round": round_num, "reason": reason, "stuck_rounds": _stuck_rounds, "max_stuck_rounds": _MAX_STUCK_ROUNDS, "runaway": _runaway, "message": _lb_message}) + "\n\n"
...
_DOMAIN_TOOL_MAP = {
    ...
    "documents": {"create_document", "edit_document", "update_document", "suggest_document", "manage_documents"},
    "email": {"list_email_accounts", "list_emails", "read_email", "scan_email_unsubscribes", "unsubscribe_email", "send_email", "reply_to_email", "bulk_email", "archive_email", "delete_email", "mark_email_read", "resolve_contact", "manage_contact", "download_attachment", "read_email_attachment", "draft_email", "draft_email_reply"},
    "cookbook": {"download_model", "serve_model", "serve_preset", "list_serve_presets", "list_served_models", "stop_served_model", "tail_serve_output", "list_downloads", "cancel_download", "search_hf_models", "list_cached_models", "list_cookbook_servers", "adopt_served_model"},
    "notes_calendar_tasks": {"manage_notes", "manage_calendar", "manage_tasks"},
    "ui": {"ui_control"},
    "sessions": {"create_session", "list_sessions", "manage_session", "send_to_session", "search_chats"},
        ...
        if not tool_blocks:
            ...
            if _looks_like_promise and _intent_nudge_count < _MAX_INTENT_NUDGES:
                ...
                if _has_holding:
                    ...
                    _matched_phrase = _intent_match.group(0).strip()
                    logger.info(f"[agent] intent-without-action nudge #{_intent_nudge_count} on round {round_num}")
